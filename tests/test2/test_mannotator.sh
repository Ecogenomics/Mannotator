...
#CMD="mannotator.pl -gffs ${RAST_GFF},${PRODIGAL_GFF} -contigs ${CONTIGS_FASTA} -protdb ${PROTDB} -i2a ${I2A} -seq_embed -threads $THREADS -out $OUT -keep"
CMD="mannotator.pl -gffs ${PRODIGAL_GFF},${RAST_GFF} -contigs ${CONTIGS_FASTA} -protdb ${PROTDB} -i2a ${I2A} -seq_embed -threads $THREADS -out $OUT -keep"
echo $CMD
eval $CMD
echo ""
